...
export default function EncuestaApp() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://${process.env.REACT_APP_URL_LOCAL}/api/encuestas`)
      .then((res) => {
  ...
  const enviarRespuestas = async () => {
    ...
    try {
      for (const [encuestaId, respuesta] of Object.entries(respuestas)) {
        await axios.post(
          `http://${process.env.REACT_APP_URL_LOCAL}/api/responder`,
          ...
          <div style={{ marginTop: "10px" }}>
            {encuesta.opciones.length === 0 ? (
              <input
                ...
                onChange={(e) => handleRespuesta(encuesta._id, e.target.value)}
                style={{
                  width: "90%",
                  padding: "8px",
              ...
              encuesta.opciones.map((opcion) => (
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid #007bff",
                    backgroundColor:
                      respuestas[encuesta._id] === opcion ? "#007bff" : "white",
                    color:
                      respuestas[encuesta._id] === opcion ? "white" : "#007bff",
                    borderRadius: "4px",
                    cursor: "pointer",
                  }}
